...
setTimeout(function(){
    if(counter < numberOfP){
        var LA = setInterval(function(){
            var points_str = "";
            var c = 0;
            for(var y = 0; y <= Math.floor(counter_p.length*counter/numberOfP); y++){
                if(counter_p[y] < 20){
                    counter_p[y]++;
                }
            }
            ...
            points.forEach(function(it, i){      
                ...
                if(i>0 && i<8){
                    points_str += counter_p[c]/20*(Number(points_g[c].split(",")[0])-Number(points[i].split(",")[0]))+Number(points[i].split(",")[0]);
                    points_str += ",";
                    points_str += counter_p[c]/20*(Number(points_g[c].split(",")[1])-Number(points[i].split(",")[1]))+Number(points[i].split(",")[1]);
                    // console.log(c+','+)
                    c++;  
                }else{
                    points_str += it;
            ...
            if(counter_p[counter_p.length-1] == 20){
                if(goodtogo == 1){
                    clearInterval(LA);
